...
export default async function AppPage() {
  // Fetch real patient appointments from backend
  ...
  return (
    <main className="bg-gray-50/60 min-h-screen flex flex-col pb-12">
      <AppHeader />

      <div className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 sm:py-7 lg:py-8 w-full space-y-6 sm:space-y-8">
        
        {/* 1. Welcome Header Banner */}
        <section className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white border border-gray-200/80 rounded-2xl p-5 sm:p-6 shadow-2xs">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <span className="inline-flex items-center justify-center w-7 h-7 rounded-lg bg-primary-50 text-primary-600">
                <Sparkles className="w-4 h-4" />
              </span>
              <h1 className="text-lg sm:text-xl lg:text-2xl font-bold text-gray-950">
                مرحباً بك في مستشفى حضرموت الحديث
              </h1>
            </div>
            <p className="text-xs sm:text-sm text-gray-500">
              خدمات رعاية صحية متقدمة وحجز مواعيد فوري مع نخبة الأطباء والاستشاريين
            </p>
          </div>
        ...
        <section className="space-y-3">
          ...
          {nextAppointment ? (
            <div className="bg-white border-2 border-primary-100 hover:border-primary-300 rounded-2xl p-4 sm:p-6 shadow-xs transition-all duration-200 relative overflow-hidden">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-3 pt-1 text-xs text-gray-500 flex-wrap">
                      <span className="inline-flex items-center gap-1 font-medium text-gray-700 bg-gray-50 px-2 py-0.5 rounded-md border border-gray-200">
                        ...
                        {formatDisplayDate(nextAppointment.appointment_date)}
                      </span>
                      <span className="inline-flex items-center gap-1 font-medium text-gray-700 bg-gray-50 px-2 py-0.5 rounded-md border border-gray-200">
            ...
            <div className="bg-white border border-gray-200 rounded-2xl p-6 text-center space-y-3 shadow-2xs">
        ...
        <section className="space-y-3">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            ...
            <Link
              href="/app/booking"
              className="bg-white border border-gray-200/90 hover:border-primary-300 rounded-2xl p-4 sm:p-5 flex flex-col justify-between transition-all duration-200 group shadow-2xs hover:shadow-xs active:scale-[0.98] select-none"
            >
            ...
            <Link
              href="/app/doctors"
              className="bg-white border border-gray-200/90 hover:border-emerald-300 rounded-2xl p-4 sm:p-5 flex flex-col justify-between transition-all duration-200 group shadow-2xs hover:shadow-xs active:scale-[0.98] select-none"
            >
            ...
            <Link
              ...
              className="bg-white border border-gray-200/90 hover:border-amber-300 rounded-2xl p-4 sm:p-5 flex flex-col justify-between transition-all duration-200 group shadow-2xs hover:shadow-xs active:scale-[0.98] select-none"
            >
            ...
            <Link
              ...
              className="bg-white border border-gray-200/90 hover:border-purple-300 rounded-2xl p-4 sm:p-5 flex flex-col justify-between transition-all duration-200 group shadow-2xs hover:shadow-xs active:scale-[0.98] select-none"
            >
              ...
              <div className="space-y-0.5">
                ...
                <p className="text-xs text-gray-500">
                  {appointments.length > 0 ? `${appointments.length} موعد مسجل` : "متابعة الحجوزات"}
                </p>
              </div>
            </Link>
              ...
              {recentAppointments.map((appointment) => {
                ...
                return (
                  <Link
                    ...
                    href={`/app/appointments/${appointment.appointment_id}`}
                    className="bg-white border border-gray-200/90 hover:border-primary-300 rounded-2xl p-4 flex items-center justify-between gap-3 transition-all duration-200 group shadow-2xs hover:shadow-xs active:scale-[0.99]"
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="w-10 h-10 rounded-xl bg-gray-50 border border-gray-100 flex items-center justify-center text-gray-600 group-hover:text-primary-600 group-hover:bg-primary-50 transition-colors shrink-0">
                        <User className="w-5 h-5" />
                      </div>
                      <div className="space-y-0.5 min-w-0">
                        <h4 className="font-bold text-gray-950 text-sm truncate group-hover:text-primary-600 transition-colors">
                          {appointment.doctor_name || "طبيب المستشفى"}
                        </h4>
                        <p className="text-xs text-gray-500 truncate">
                          {formatDisplayDate(appointment.appointment_date)} • {appointment.period}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <ChevronLeft className="w-4 h-4 text-gray-400 group-hover:text-primary-600 transition-colors" />
                    </div>
                  </Link>
                );
              })}
        ...
        <section className="bg-white border border-gray-200/90 rounded-2xl p-4 sm:p-5 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-2xs">
